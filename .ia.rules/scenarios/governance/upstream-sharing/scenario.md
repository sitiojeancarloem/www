...
Extensão de `./AGENTS.md` §§0, 17–18; aplicar `MN-ROLE`, `MN-EXT`, `MN-REF`, `MN-OUT`, `MN-CMD`, `MN-API`, `MN-DEF`, `MN-STATE`, `MN-VAL` e `./.ia.rules/core/contracts.md`. Aplica-se ao consumidor que identifica contribuição reutilizável e ao construtor somente para inbox/avaliação formal de issue; não transfere autoridade.
...
Comandos aplicam `./.ia.rules/core/contracts.md` CT-4, `MN-DEF` e `to-ia`. Falha externa DEVE preservar proposta e permitir dry-run/offline. Validar papéis, fontes, atualização, duplicação, recusa, sanitização, autorização, registro e ausência de alteração no núcleo do consumidor.
