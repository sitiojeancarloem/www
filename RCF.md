...
- Compartilhamento adicional ou exceção DEVE ser deliberado, nominal, justificado e versionado neste RCF ou no contrato do adaptador, com teste que demonstre por que a equivalência é necessária nos dois contextos. Coincidência de seletor, classe, propriedade, aparência ou estrutura existente NÃO constitui autorização.
- A integração Jekyll declara somente estas exceções atuais: conteúdo e semântica editorial; abertura de `Referências` e `Bibliografia`; redução explícita de colunas Markdown internas para uma coluna antes da composição IEEE; preservação controlada de cores de imagens e tabelas editoriais. Nenhuma decoração web de painel, título, parágrafo, tema ou chrome integra essa lista.
