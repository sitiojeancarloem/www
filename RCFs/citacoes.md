...
- `framed-accent` DEVE preservar moldura lateral e inferior, superfície interna, cabeçalho destacado e adorno lateral; `pull-quote` DEVE preservar aspas grandes laterais, corpo alinhado à esquerda, autoria inferior à esquerda e pequeno recuo do conjunto; `centered-mark` DEVE preservar aspas superiores, corpo estreito centralizado e ausência de fundo externo; `editorial-statement` DEVE preservar aspas superiores, declaração forte centralizada, autoria inferior e ausência de fundo externo fixo. [PENDENTE-CODIGO]
- `thematic-rail` DEVE preservar corpo e autoria alinhados à esquerda, duas hastes verticais colineares e aspas visíveis dentro do intervalo entre elas. O intervalo e as hastes DEVEM possuir geometria própria estável, permanecer verticalmente centralizados e alinhados com as aspas e NÃO DEVEM mudar de proporção ou posição conforme quantidade, quebra ou altura do texto. [PENDENTE-CODIGO]
