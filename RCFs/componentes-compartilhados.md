...
- O material visual DEVE pertencer ao conjunto inteiro e formar vidro fumê contínuo através das duas regiões: alpha, densidade e blur preservam o baseline visual vigente ajustado pelo desenvolvedor; o conteúdo posterior permanece visivelmente desfocado por `backdrop-filter` nativo, sem superfície opaca, cópia da COVER, gradiente histórico reintroduzido ou dupla filtragem. A região superior permanece vazia e sobreposta; a inferior contém exclusivamente o título. A junção NÃO PODE exibir gap, degrau, faixa artificial ou mudança de largura em claro, escuro, mobile ou desktop, e fallback sem suporte a blur DEVE preservar contraste e integração. [PENDENTE-CODIGO]
- A sombra do conjunto DEVE existir em camada externa ao material e projetar-se somente para baixo, sobre o conteúdo posterior à barra. Ela NÃO PODE sombrear a COVER, a face interna do vidro, o título ou a flag, alterar o backdrop nem substituir o blur do conteúdo realmente localizado atrás da superfície. [61d4836]
...
- COVER, região superior, região inferior, flag e metadados subsequentes DEVEM preservar a ordem estrutural e de pintura `masthead -> mídia disponível e dimensionada -> material vítreo composto -> upper + flag -> lower + title -> metadados/conteúdo`. Somente a região superior e a parcela da flag anterior à sua sustentação PODEM interpenetrar visualmente a faixa terminal da COVER; foreground permanece acima do material, sem `filter` ou `backdrop-filter`, e nenhuma sobreposição PODE ser ocultada ou simulada apenas por `z-index`. Revalidação local do compositor, quando necessária no Chromium, NÃO PODE alterar o estilo final, piscar, escapar do cabeçalho editorial nem persistir trabalho na main thread. [PENDENTE-CODIGO]
...
- A validação renderizada DEVE medir a colinearidade entre essa sustentação e `upper-bar.top`, a sobreposição exclusiva da região superior, sua profundidade estrutural acrescida pelo deslocamento intrínseco da FLAG, a continuidade com a região inferior e a ausência de clipping em hero legado, `content`, `wide single`, `wide triptych` e modos estendidos. Também DEVE conferir no material único os estilos computados de alpha e blur/backdrop contra o baseline manual vigente e a sombra externa; no foreground, nitidez e ausência de filtros; e no título, amarelo, `text-shadow`, ausência de decoração/pseudoícone e hierarquia da flag em claro/escuro, mobile/desktop, DPR, resize, orientação, scroll e impressão. A prova DEVE incluir página editorial real, primeiro paint/reload sem toggle manual, mudança da região do COVER atrás do vidro durante resize/orientação e distinção explícita entre modos colineares à zona do artigo e modos full-width. [PENDENTE-CODIGO]
