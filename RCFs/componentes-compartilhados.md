...
- O material visual DEVE pertencer ao conjunto inteiro e formar vidro fumê contínuo através das duas regiões: gradiente RGBA perceptível de mais translúcido para visualmente mais sólido sem perder alpha, conteúdo posterior visivelmente desfocado por `backdrop-filter` nativo e nenhuma superfície opaca ou cópia da COVER. A região superior permanece vazia e sobreposta; a inferior contém exclusivamente o título. A junção NÃO PODE exibir gap, degrau, faixa artificial, dupla filtragem ou mudança de largura em claro, escuro, mobile ou desktop, e fallback sem suporte a blur DEVE preservar contraste e integração. [PENDENTE-CODIGO]
- A sombra do conjunto DEVE existir em camada externa ao material e projetar-se somente para baixo, sobre o conteúdo posterior à barra. Ela NÃO PODE sombrear a COVER, a face interna do vidro, o título ou a flag, alterar o backdrop nem substituir o blur do conteúdo realmente localizado atrás da superfície. [PENDENTE-CODIGO]
...
- COVER, região superior, região inferior, flag e metadados subsequentes DEVEM preservar a ordem estrutural e de pintura `masthead -> cover -> material vítreo contínuo -> upper + flag -> lower + title -> metadados/conteúdo`. Somente a região superior e a parcela da flag anterior à sua sustentação PODEM interpenetrar visualmente a faixa terminal da COVER; foreground permanece acima do material, sem `filter` ou `backdrop-filter`, e nenhuma sobreposição PODE ser ocultada ou simulada apenas por `z-index`. [PENDENTE-CODIGO]
...
- A validação renderizada DEVE medir a colinearidade entre essa sustentação e `upper-bar.top`, a sobreposição exclusiva da região superior, sua profundidade estrutural acrescida pelo deslocamento intrínseco da FLAG, a continuidade com a região inferior e a ausência de clipping em hero legado, `content`, `wide single`, `wide triptych` e modos estendidos. Também DEVE conferir no material único os estilos computados de alpha, blur/backdrop, gradiente contínuo e sombra externa; no foreground, nitidez e ausência de filtros; e no título, amarelo, `text-shadow`, ausência de decoração/pseudoícone e hierarquia da flag em claro/escuro, mobile/desktop, DPR, resize, orientação, scroll e impressão. A prova DEVE incluir página editorial real e mudança da região do COVER atrás do vidro durante resize/orientação. [PENDENTE-CODIGO]
