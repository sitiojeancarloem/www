...
- Cabeçalho editorial de artigo DEVE usar uma única estrutura compartilhada para título, duas barras e flag, independentemente do modo de COVER; modalidade visual só PODE alterar tokens ou posicionamento previstos, nunca duplicar o conjunto. [86b8972]
- A barra inferior DEVE ser sólida e conter exclusivamente o título editorial dentro do conjunto; a barra superior NÃO PODE conter o título e DEVE transicionar verticalmente de RGBA sutilmente translúcido no topo para a mesma cor sólida da barra inferior na base, sem emenda, salto ou faixa artificial. Ambas DEVEM respeitar a largura útil do cabeçalho e permanecer alinhadas em claro, escuro, mobile e desktop. [PENDENTE-CODIGO]
- A flag DEVE iniciar depois da COVER, alinhada ao conjunto de barras e integralmente contida na região vertical posterior à fronteira comum; sua referência geométrica da dobra triangular DEVE ser colinear ao topo da barra superior. Dobra, proporção e conteúdo permanecem íntegros, sem corte, gap, sobreposição com a cover ou clipping por stage, wrapper, pattern, Hero ou viewport. [PENDENTE-CODIGO]
- Cover, duas barras, flag e metadados subsequentes DEVEM seguir a ordem estrutural `masthead → cover → flag + barras → metadados/conteúdo`; `z-index` isolado NÃO PODE ocultar interpenetração geométrica, e o conjunto de barras NÃO PODE ser elevado por margem negativa, transformação ou offset para invadir a cover. [PENDENTE-CODIGO]
...
- O HTML final DEVE provar que o título é descendente direto da barra inferior, que a barra superior não contém título, que a flag usa a mesma referência geométrica do topo da barra superior e que metadados de leitura permanecem posteriores ao conjunto de duas barras. [PENDENTE-CODIGO]
