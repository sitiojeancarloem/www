...
- Cada modelo registrado DEVE possuir ao menos uma representação visual fiel à implementação e, nos cinco modelos derivados de `e1.png`–`e6.png`, fiel também à evidência canônica estrita e às exceções expressas de `RCFs/citacoes.md`. Se um mesmo modelo oferecer somente variação de cor ou somente variação de ícone, duas amostras distintas bastam; combinação nomeada de múltiplas cores DEVE exibir paleta compacta com os tons reais. [PENDENTE-CODIGO]
