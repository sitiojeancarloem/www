...
- `featured_image_style: wide` ativa exclusivamente a composição horizontalmente infinita; os aliases legados `full`, `full-width` e `bleed` convergem ao mesmo modo. Somente essa modalidade PODE usar largura de viewport, sangria lateral e superfície periférica para completar a faixa sem amputar o conteúdo central.
...
- No modo `content`, a mídia central permanece contida na caixa local e a superfície visível permanece limitada à zona do artigo. Quando proporção e limites de altura impedirem a mídia de tocar simultaneamente os quatro lados, a compensação local NÃO PODE transformar a caixa em faixa de viewport, distorcer, recortar destrutivamente nem alterar a semântica visual.
- Flag associada à publicação pertence à camada acima da cover e DEVE permanecer integralmente visível em `content` e `wide`. Cover, skeleton, contenção ou novo stacking context NÃO PODEM cobri-la nem recortá-la.
...
- O modo `wide` de uma imagem, selecionado por ausência de `header.image_wide_mode` ou por `single`, usa `header.image` como conteúdo central autoritativo, mantém a imagem centralizada em relação à página e PODE completar periferias por camada derivada da mesma fonte, desde que a imagem integral continue perceptível e a composição não introduza faixa vazia, costura, deslocamento ou semântica visual nova.
