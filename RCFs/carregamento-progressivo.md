...
- A arquitetura preserva modalidades cumulativamente distintas: o hero legado sem estilo explícito; a cover comum `content`, alinhada à zona visível do artigo; a cover horizontalmente infinita `wide` de uma imagem; e a variante opcional `wide` tripla. Ausência de estilo NÃO PODE ativar `wide`, e regra de uma modalidade NÃO PODE vazar para outra.
- `featured_image_style: content` ativa exclusivamente a cover comum; o alias legado `inline` converge ao mesmo modo. Sua caixa DEVE coincidir com as bordas visíveis da zona do artigo, sem `100vw`, margem de sangria ou superfície periférica de página, preservando proporção, enquadramento e conteúdo integral.
- `featured_image_style: wide` ativa exclusivamente a composição horizontalmente infinita; os aliases legados `full`, `full-width` e `bleed` convergem ao mesmo modo. Somente essa modalidade PODE usar largura de viewport e sangria lateral, segundo o enquadramento próprio de `single` ou a composição própria de `triptych`.
...
- O layout e a implementação DEVEM preservar a modalidade já declarada por cada publicação. Correção localizada de largura, altura, flag, asset ou metadado NÃO autoriza reclassificar post, preencher estilo ausente com `wide`, criar default global infinito nem remover modalidade preexistente.
...
- A validação DEVE aferir separadamente hero legado, `content`, `wide single` e `wide triptych` em claro/escuro, orientações retrato/paisagem e viewports representativas; comprovar seleção, largura da zona aplicável, limites de altura, flag integral, centralização, proporção, crop natural do `wide single`, continuidade exclusiva do `triptych`, ausência de blur/superfície duplicada e ausência de vazamento entre modalidades.
- O hero legado DEVE ser exercitado em viewport no qual caiba integralmente e em viewport no qual alterne para a zona do artigo. O teste registra geometria antes e depois de resize, posição de scroll invariável, quantidade limitada de decisões e ausência de oscilação, flicker, cover parcialmente invisível ou escrita de layout em alta frequência.
- A impressão DEVE conservar a política vigente para imagens destacadas sem deixar superfície, segmento lateral, altura de tela ou sangria de uma modalidade interferir na composição impressa.
