...
- Citação em bloco semanticamente reconhecida, independentemente de ser materializada por `<blockquote>`, `role="blockquote"` ou `data-jcem-blockquote`, DEVE receber exatamente uma fronteira falada `Início da citação.` / `Fim da citação.` por bloco externo quando necessária à compreensão; seletores coexistentes no mesmo elemento NÃO DEVEM duplicar a fronteira, citação inline usa indicação breve integrada e subcitação preserva a hierarquia sem repetir marcadores em cascata a cada nó. [PENDENTE-CODIGO]
...
- Toda referência bíblica reconhecida DEVE ser interpretada em uma única estrutura de livro, grupos, capítulos, versículos, intervalos, separadores, conectivos e versão antes de gerar suas formas faladas curta e longa; artigo, rota, livro ou exemplo específico NÃO PODE determinar o comportamento. [b3095f0]
- `:` DEVE representar exclusivamente a relação estrutural entre capítulo e versículo e NÃO PODE produzir `para`, `por`, `até` ou outra palavra na fala; horário, URL, código, fórmula e sintaxe não classificados como referência bíblica permanecem inalterados. [b3095f0]
- A forma curta DEVE verbalizar livro, capítulo e versículos por números e pausas proporcionais aos separadores, usando somente `a` em intervalo e conectivo explícito ou inequivocamente exigido pela lista; `Gênesis 2:7` resulta em `Gênesis, 2, 7`, sem anunciar `capítulo` ou `versículo`. [b3095f0]
- A forma longa DEVE explicitar `capítulo`, `versículo` ou `versículos`, usar `de <início> a <fim>` para intervalo, naturalizar o último conectivo de lista e repetir `capítulo` em cada mudança de grupo sem repetir livro herdado; `Gênesis 2:7` resulta em `Gênesis, capítulo 2, versículo 7`. [b3095f0]
- Vírgula entre versículos, `e` explícito, hífen de intervalo e ponto e vírgula entre grupos DEVEM conservar ordem, contexto herdado e prosódia por vírgula, conectivo, `a` e ponto e vírgula respectivamente; inferência só PODE naturalizar lista inequivocamente analisada e NÃO PODE preencher construção inválida ou indefinida. [b3095f0]
- A cobertura permanente DEVE combinar livros simples, compostos, numerados e abreviados com referências simples e compostas, intervalos, versículos avulsos, mudanças de capítulo, grupos, conectivos explícitos/inferidos e versões, comprovando no payload efetivo os modos curto/longo e a ausência de palavra artificial entre capítulo e versículo. [b3095f0]
