...
Escopo: artigos que sintetizam bate-papos, conversas, estudos dialogados, gravações, áudios transcritos ou qualquer conteúdo conversacional equivalente, independentemente do meio de captura.
...
- Como a síntese não possui autor originário cuja forma literária deva ser imitada, ela NÃO DEVE fabricar uma voz autoral nem reproduzir vícios da transcrição. Isso não altera a obrigação de preservar autoria intelectual, intenção, intensidade, divergências, hesitações e formulações materialmente relevantes dos participantes. [PENDENTE-CODIGO]
- A linguagem deve ser acessível a públicos com diferentes níveis de formação, inclusive baixa escolaridade e provável analfabetismo funcional, conforme o rigor acessível de `.ia.rules/resources/editorial-authoring.md`. Palavra simples igualmente precisa é preferida; termo técnico necessário recebe, na primeira ocorrência pertinente, explicação curta, simples e correta, sem repetição automática nem rebuscamento dispensável. [PENDENTE-CODIGO]
...
- A simplificação atua na linguagem e na organização, nunca na substância. Ela NÃO PODE reduzir força, nuance, complexidade, raciocínio intermediário, condicionais ou referências; assunto complexo permanece tratado com plenitude compatível com a fonte. [PENDENTE-CODIGO]
- A primeira ocorrência explícita de cada citação textual ou referência inequívoca feita no bate-papo deve apresentar integralmente o trecho citado e sua referência nomeada `[^id]`; mera referência bibliográfica, título desacompanhado do texto, resumo ou paráfrase NÃO substitui a citação.
- Quando a fala ou a transcrição cortar, entrecortar, corromper ou omitir parte do trecho citado, a edição DEVE localizar uma fonte confiável, recompor a íntegra material e identificar a intervenção editorial e sua fonte; a recomposição NÃO PODE ampliar o argumento além da passagem efetivamente indicada.
- Se a passagem exata não puder ser localizada com segurança, a pendência DEVE permanecer explícita e bloqueia a publicação do conteúdo até resolução, sem preenchimento conjectural.
...
- Quando participante fizer referência inequívoca a fonte ou citação omitida, cortada ou não localizada, a recomposição integral prevista acima é obrigatória, identificada como intervenção editorial rastreável e sem expansão livre do argumento.
...
- A validação editorial deve confirmar o aviso na abertura, caráter temático, anonimização, linguagem acessível, preservação proporcional do conteúdo e da autoria intelectual, ausência das referências processuais vedadas fora da exceção documental, ausência de presunção coletiva, distinção entre conteúdo documentado e elaboração editorial, ausência de mera referência desacompanhada da citação integral, recomposição rastreável de cortes e ausência de repetição textual desnecessária.
