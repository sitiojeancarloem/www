...
class SilktideCookieBanner {
	...
	// ----------------------------------------------------------------
	createBackdrop() {
		this.backdrop = this.createWrapperChild(
			null,
			'silktide-backdrop',
		);
	}
	...
	getAcceptedCookies() {
		return (this.config.cookieTypes || []).reduce(
			(acc, cookieType) => {
				acc[cookieType.id] =
	...
	getBannerContent() {
		...
		const acceptAllButton = `<button class="accept-all st-button st-button--primary"${
			...
			acceptAllButtonLabel !== acceptAllButtonText
	...
	createBanner() {
		...
		if (
			this.banner &&
			typeof this.config.onBannerOpen === 'function'
		) {
	...
	createCookieIcon() {
		...
		if (this.cookieIcon && this.config.cookieIcon?.colorScheme) {
			this.cookieIcon.classList.add(
				this.config.cookieIcon.colorScheme,
			);
		}
	}
	...
	setupEventListeners() {
		...
		if (this.banner) {
			...
			const preferencesButton =
				this.banner.querySelector('.preferences');
