...
assert.match(publicationWorkflow, /CI_TOTAL_TIMEOUT_SECONDS:\s*900/);
assert.match(publicationWorkflow, /jekyll:[\s\S]*?timeout-minutes:\s*15/);
assert.match(publicationWorkflow, /Install Node dependencies[\s\S]*?ci_timed_step\.rb "Install Node dependencies" 240 -- npm ci/);
