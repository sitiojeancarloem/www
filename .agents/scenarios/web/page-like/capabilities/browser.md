...
Predicado: entrega executada ou consumida por navegador/engine web. Depende do roteador `../scenario.md`; aplicar `W-MTX-42`, `MN-API`, `MN-DEF`, `MN-OUT`, `MN-CMD` e `./.agents/core/contracts.md`. Regra condicional deste módulo só se ativa quando RCF/configuração a declarar ou a interface existente já implementar o recurso; ausência de ambas NÃO autoriza introduzi-lo.
...
É capacidade composta de borda, não classe-base: navegador, saída estática, hospedagem, editorial e demais capacidades DEVEM ser ordenadas conforme `./.agents/core/contracts.md`. Jekyll, GitHub Pages, Git, gerador e hospedagem são independentes; coincidência de ferramenta NÃO prova dependência. `W-MTX-42` expande exatamente 42 combinações e os contratos `Dev,Prod,Lib,Bundle,Offline`.
